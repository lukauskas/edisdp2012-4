    ...
    public AbstractMessage decodeMessage(int hashedMessage) throws InvalidArgumentException {
        switch (AbstractMessage.extactOpcodeFromEncodedMessage(hashedMessage)) {
            ...
            case MessageMove.OPCODE:
                int[] moveArguments = MessageMove.decodeArgumentsFromHash(hashedMessage);
                return new MessageMove(moveArguments[0] - MessageMove.OFFSET, moveArguments[1]
                        - MessageMove.OFFSET);
            case MessageRotate.OPCODE:
                int[] rotateArguments = MessageRotate.decodeArgumentsFromHash(hashedMessage);
                return new MessageRotate(rotateArguments[0] - MessageRotate.ANGLE_OFFSET,
                        rotateArguments[1] - MessageRotate.ANGLE_OFFSET);
            case MessageStop.OPCODE:
                int floatWheels = MessageStop.decodeArgumentsFromHash(hashedMessage);
