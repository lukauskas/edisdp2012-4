...
public class BasicWorld extends AbstractWorld {
	...
	void interpret(double yPosX, double yPosY, double yRad, double bPosX,
		...
		if (isBlue()) {
			oPosX = bPosX; oPosY = bPosY; oRad = bRad;
			tPosX = yPosX; tPosY = yPosY; tRad = yRad;
		} else {
			oPosX = yPosX; oPosY = yPosY; oRad = yRad;
			tPosX = bPosX; tPosY = bPosY; tRad = bRad;
		}
		...
		if (prev == null) {	
			...
			oursDPos = (new Coord(oPosX, oPosY)).sub(prev.getBalle().getPosition());
			themDPos = (new Coord(tPosX, tPosY)).sub(prev.getOpponent().getPosition());
			ballDPos = (new Coord(ballPosX, ballPosY)).sub(prev.getBall().getPosition());
			...
			ball = new FieldObject(new Coord(ballPosX,ballPosY), ballAngle, ballVel);
		}
