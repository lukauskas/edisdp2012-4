		...
		if (world != null) {
			...
			if(s != null) {
				if (ball!= null) {
					...
					ball.getPosition().set((float)s.getBall().getPosition().getX()*scale, (float)(1.22-s.getBall().getPosition().getY())*scale);
				}
				
				if (yellow != null) {
					balle.world.Robot robot = world.isBlue()?s.getOpponent():s.getBalle();
					Coord pos = robot.getPosition();
					float angle = (float) robot.getOrientation();
					Body b = yellow.getBody();
					b.setLinearVelocity(new Vec2(0,0));
					b.getPosition().set((float)pos.getX()*scale, (float)(1.22-pos.getY())*scale);
